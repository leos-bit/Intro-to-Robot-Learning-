...
\begin{abstract}
This project studies autonomous quadrotor pathing in a randomized MuJoCo simulation. The task is to navigate from a random initial state to a random 3D goal while avoiding capsule obstacles, respecting a safety envelope, and stabilizing near the target. We evaluate model-free on-policy reinforcement learning with Proximal Policy Optimization (PPO), model-free off-policy reinforcement learning with Soft Actor-Critic (SAC), a preliminary model-based PETS/MPC result, and two final-project modifications: goal-hover reward shaping and wind-disturbance robustness. Current results show that model-free RL learns meaningful goal-directed and obstacle-aware behavior, but the task remains only partially solved. The best PPO goal-hover policy reaches a mean evaluation return of $366.8$ and a success rate of $45.3\%$, while the wind-trained PPO policy reaches $267.6$ return and $35.9\%$ success under disturbed dynamics.
\end{abstract}
...
Figure~\ref{fig:main-results} shows the current learning curves available in the repository. The figure includes the random-agent baseline, PPO baseline, SAC baseline, a provisional model-based RL estimate, Modification 1, and Modification 2. PPO, SAC, and wind PPO runs that end before the longest Mod 1 run are extended with dashed hold-forward segments so all methods are visible on the same training-step axis. The MBRL curve is marked as an estimate because the available file reports cumulative return within an MPC episode, not repeated training evaluations.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.97\linewidth]{figs/current_final_results.png}
    \caption{Mean evaluation return versus training environment steps for the random baseline, PPO baseline, SAC baseline, provisional PETS/MPC model-based estimate, goal-hover PPO modification, and wind-trained PPO modification. Dashed tails indicate hold-forward extensions for shorter completed runs.}
...
Before submission, the provisional MBRL/PETS estimate should be replaced with a true training-evaluation curve if that experiment becomes available. The project website should also be updated with videos for PPO, SAC, MBRL, Modification 1, and Modification 2. Future work should focus on curriculum learning over obstacle count and target distance, stronger final-approach damping, explicit safety constraints, hybrid RL/MPC controllers, and evaluation on held-out obstacle distributions and stronger wind settings.
...
\begin{itemize}[leftmargin=1.2em]
    \item Replace the provisional MBRL/PETS estimate with a true training-evaluation curve if one becomes available.
