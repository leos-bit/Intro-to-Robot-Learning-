...
Figure~\ref{fig:main-results} shows the current learning curves available in the repository. The figure includes the random-agent baseline, PPO baseline, SAC baseline, a provisional model-based RL estimate, Modification 1, and Modification 2. Each PPO/SAC curve uses only saved evaluation points from its corresponding training run. The MBRL curve is marked as an estimate because the available file reports cumulative return within an MPC episode, not repeated training evaluations.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.97\linewidth]{figs/current_final_results.png}
    \caption{Mean evaluation return versus training environment steps for the random baseline, PPO baseline, SAC baseline, provisional PETS/MPC model-based estimate, goal-hover PPO modification, and wind-trained PPO modification.}
