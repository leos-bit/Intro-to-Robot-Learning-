...
\begin{abstract}
This project studies autonomous quadrotor pathing in a randomized MuJoCo simulation. The task is to navigate from a random initial state to a random 3D goal while avoiding capsule obstacles, respecting a safety envelope, and stabilizing near the target. We evaluate model-free on-policy reinforcement learning with Proximal Policy Optimization (PPO), model-free off-policy reinforcement learning with Soft Actor-Critic (SAC), and two final-project modifications: goal-hover reward shaping and wind-disturbance robustness. Current results show that model-free RL learns meaningful goal-directed and obstacle-aware behavior, but the task remains only partially solved. The best PPO goal-hover policy reaches a mean evaluation return of $366.8$ and a success rate of $45.3\%$, while the wind-trained PPO policy reaches $267.6$ return and $35.9\%$ success under disturbed dynamics. Model-based RL results will be added once the PETS/MPC experiment is complete.
\end{abstract}
...
The merged repository now includes a model-based RL implementation under \texttt{jax\_implementation/MBRL/dynamics}. The planned method is PETS-style ensemble dynamics learning followed by MPC planning with CEM. The dynamics model is pretrained from collected environment data, then updated with online rollouts during MPC evaluation. The quantitative MBRL learning curve is still pending and must be added to the final results figure before submission.
...
Figure~\ref{fig:main-results} shows the current learning curves available in the repository. The figure includes the random-agent baseline, PPO baseline, SAC baseline, Modification 1, and Modification 2. The MBRL curve is not yet included because that experiment is still pending. The final submission must add the model-based RL curve to this same figure.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.97\linewidth]{figs/current_final_results.png}
    \caption{Mean evaluation return versus training environment steps for the random baseline, PPO baseline, SAC baseline, goal-hover PPO modification, and wind-trained PPO modification. The MBRL curve is pending and should be added to this same plot before final submission.}
...
Model-based RL & TODO & TODO & TODO & TODO \\
\bottomrule
\end{tabular}
\end{table}
...
Before submission, the MBRL/PETS curve should be added to Figure~\ref{fig:main-results}, Table~\ref{tab:results}, and the analysis. The project website should also be updated with videos for PPO, SAC, MBRL, Modification 1, and Modification 2. Future work should focus on curriculum learning over obstacle count and target distance, stronger final-approach damping, explicit safety constraints, hybrid RL/MPC controllers, and evaluation on held-out obstacle distributions and stronger wind settings.
...
\begin{itemize}[leftmargin=1.2em]
    \item Add the MBRL/PETS learning curve to Figure~\ref{fig:main-results} once the experiment is complete.
    \item Add MBRL best/final return, success rate, final distance, and failure analysis to Table~\ref{tab:results} and the results text.
